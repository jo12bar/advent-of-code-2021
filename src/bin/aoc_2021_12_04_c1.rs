...
fn main() -> anyhow::Result<()> {
    ...
    let mut winning_board_idx = None;
    ...
    if let Some(winning_board_idx) = winning_board_idx {
...
fn get_problem_input() -> anyhow::Result<(Vec<u16>, Vec<Board>)> {
    let mut input_file =
        get_input_file(2021, 12, 4, 1).context("Could not open problem input for reading.")?;
